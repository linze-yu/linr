...
```{r, 代码全局设置, echo = T}
knitr::opts_chunk$set(warning = F, error = F, message = F, prompt = F, comment = "", echo = T, eval = F, dpi = 200, collapse = F, fig.align = "left", dev = "png", prompt = F, cache = T)
# , eval = F 代码显示不运行
...
# 安装加载的包
```{r}
install.packages("rio") # 导入数据
install.packages("tidyfst") # dplyr类似,底层使用datatable
install.packages("data.table") # 处理大数据
install.packages("tidyverse") # tidy-R
install.packages("future") # 多核计算
install.packages("furrr") # purrr
install.packages("survminer") # 生存曲线
install.packages("survival") # 生存曲线
install.packages("ggpmisc") # 拟合模型相关的注释和绘图
install.packages("gghalves") # 裁剪图形
install.packages("ggsignif") # 添加P值
install.packages("ggforce") # 形状曲线
devtools::install_github("zzawadz/customLayout") # 图片排版
install.packages("glmnet") # LASSO回归
install.packages("circlize") # 圈图
devtools::install_github("jokergoo/ComplexHeatmap") # 热图*
install.packages("ggwordcloud") # 词云
install.packages("knitr") # 表格+渲染
install.packages("kableExtra") # 表格+
install.packages("gtsummary") # 基线资料表
install.packages("shiny") # 交互网页
install.packages("scales") # 缩放刻度
install.packages("pacman") # 参考文献
install.packages("Cairo") # 渲染
install.packages("ggbreak") # 坐标轴截断
install.packages("jiebaR") # 分词
install.packages("readtext") # 读取文本pdf,docx,xml,json
install.packages("skimr") # 用最少的代码来获知数据框的方方面面
install.packages("tokenizers") # 文本切分
...
install.packages("DMwR2") # 使用局部异常因子法(LOF法)检测异常值
install.packages("styler") # 整理代码
install.packages("rvcheck") # 更新包
install.packages("ggwordcloud") # 词云
install.packages("showtext") # 中文PDF
install.packages("DiagrammeR") # 流程图
install.packages("ggrepel") # 文本防重叠
```

# 安装本地包customLayout&ComplexHeatmap
```{r}
setwd("D:/desktop")
...
lin_theme <- theme(
  ...
  axis.ticks.length = unit(.15, "cm"), # 刻度线长度
  panel.grid.major.x = element_line(color = "#96999C", size = 0.5, linetype = 3, lineend = "round"), # 主竖线
  panel.grid.major.y = element_line(color = "#96999C", size = 0.5, linetype = 3, lineend = "round"), # 主横线
  panel.grid.minor.x = element_line(color = "#96999C", size = 0.25, linetype = 3, lineend = "round"), # 次竖线
  panel.grid.minor.y = element_line(color = "#96999C", size = 0.25, linetype = 3, lineend = "round"), # 次横线
  plot.margin = margin(t = 0.5, r = 0.5, b = 0.1, l = 0.5, unit = "cm"), # 图边界(距离可以为0或者为负值)
...
### 整理代码
```{r,整理代码}
cclean <- print("styler:::style_active_file()")
```
...
remotes::install_github("juba/rmdformats") # Rmarkdown主题
...
remotes::install_github("brentthorne/posterdown") # 海报
...
remotes::install_github("r-lib/styler") # 整理代码
devtools::install_github("gadenbuie/rsthemes") # Rstudio主题
install.packages("DiagrammeR") # 流程图
remotes::install_github("DavisVaughan/furrr") # 并行purrr
remotes::install_github("davidgohel/officedown") # officeverse
devtools::install_github("davidgohel/officer") # officeverse
devtools::install_github("ardata-fr/mschart") # officeverse
devtools::install_github("davidgohel/flextable") # officeverse
devtools::install_github("davidgohel/rvg") # officeverse
```
...
install.packages("ggrepel") # 文本防重合
```
...
## 其他
```{r}
remotes::install_github("xoopR/param6") # 快速轻便的 R6 参数接口
...
remotes::install_github("wviechtb/metafor") # Meta分析
```
...
remotes::install_github("rich-iannone/stationaRy") # 从成千上万个全球站点收集到的每小时气象数据
devtools::install_github("jennybc/gapminder") # 摘自Gapminder的数据
devtools::install_github("juliasilge/janeaustenr") # 简·奥斯丁小说全集数据
remotes::install_github("wjakethompson/taylor") # Taylor Swift 歌曲的精选数据集的轻松访问，包括歌词和音频特征
```
...
remotes::install_github("tidymodels/infer") # 5
...
remotes::install_github("mlr-org/mlr3filters") # mlr3的变量选择过滤器
remotes::install_github("mlr-org/mlr3fselect") # mlr3的包装器功能选择
remotes::install_github("mlr-org/mlr3learners", dependencies = TRUE) # 推荐的学习器mlr3
remotes::install_github("mlr-org/mlr3proba") # 监督概率学习的扩展(包括生存分析)
remotes::install_github("mlr-org/mlr3tuning") # 通过随机搜索,网格搜索等进行超参数调优
remotes::install_github("mlr-org/mlr3viz") # mlr3的可视化效果
remotes::install_github("mlr-org/paradox") #
remotes::install_github("mlr-org/mlr3extralearners") #
remotes::install_github("mlr-org/bbotk") # 黑盒优化工具包
remotes::install_github("mlr-org/mlr3benchmark") # mlr3中用于基准测试的分析和工具
devtools::install_github("mlr-org/mlr3db") # 使用数据后端扩展mlr3包
remotes::install_github("mlr-org/mlr3hyperband") # mlr3的超频调谐
# mlr3measures#连接器到OpenML
# mlr3oml#用于预处理和构建复杂工作流的管道和DAG
remotes::install_github("mlr-org/mlr3spatiotempcv") # 用于对时空任务进行重新采样的扩展
...
remotes::install_github("const-ae/ggsignif") # 添加P值,CRAN
...
## 其他类型R包
```{r}
install.packages("showtext") # pdf中文
...
remotes::install_github("ShixiangWang/ezcox") # 很方便的做Cox回归分析模型和森林图
install.packages("ggstream") # 河流图绘制
remotes::install_git("appliedepi/epirhandbook") # 该包集合了流行病学中诸多描述性分析的函数,其中epi.tests函数专门用于计算诊断试验的灵敏度,特异度及预测值等指标,详细介绍可以看链接https://rpubs.com/maomaoworm/690424
